...
template<typename DType>
inline void MultiBoxDet2TargetForward(const Tensor<gpu, 2, DType> &loc_target,
  ...
  cuda::CheckLaunchParam(init_block_dim, init_thread_dim, "MultiBoxDet2Target Init");
  ...
  cuda::CheckLaunchParam(num_batches, num_threads, "MultiBoxDet2Target Matching");
  ...
  if (negative_mining_ratio > 0) {
    ...
    cuda::CheckLaunchParam(num_blocks, num_threads, "MultiBoxDet2Target Negative");
...
namespace op {
template<>
Operator *CreateOp<gpu>(MultiBoxDet2TargetParam param, int dtype) {
  Operator *op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new MultiBoxDet2TargetOp<gpu, DType>(param);
  });
  return op;
}
}  // namespace op
