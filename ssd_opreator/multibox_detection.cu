...
template<typename DType>
__global__ void DetectionForwardKernel(DType *out, DType *outanchor,const DType *cls_prob,
  ...
  outanchor += nbatch * num_anchors * 6;
  ...
  for (int i = index; i < num_anchors; i += blockDim.x) {
    ...
    for (int j = 1; j < num_classes; ++j) {
      ...
      out[pos * 6 + 4] = xmax;
      out[pos * 6 + 5] = ymax;
      outanchor[pos * 6 ]=out[pos * 6 ];
      outanchor[pos * 6 + 1]=out[pos * 6 + 1];
      outanchor[pos * 6 + 2]=out[pos * 6 + 2];
      outanchor[pos * 6 + 3]=out[pos * 6 + 3];
      outanchor[pos * 6 + 4]=out[pos * 6 + 4];
      outanchor[pos * 6 + 5]=out[pos * 6 + 5];
...
inline void MultiBoxDetectionForward(const Tensor<gpu, 3, DType> &out,
                                     const Tensor<cpu, 3, DType> &outanchor,
  ...
  cuda::DetectionForwardKernel<<<num_blocks, num_threads, 0, stream>>>(out.dptr_, outanchor.dptr_
