...
enum MultiBoxDetectionOpOutputs {kOut, kOutAnchor};
...
class MultiBoxDetectionOp : public Operator {
 public:
  ...
  virtual void Forward(const OpContext &ctx,
     ...
     CHECK_EQ(out_data.size(), 2U) << "Output: [Out, OutAnchor]";
     ...
     Tensor<xpu, 3, DType> out = out_data[mboxdet_enum::kOut]
      ...
      Tensor<xpu, 3, DType> outanchor = out_data[mboxdet_enum::kOutAnchor]
       .get<xpu, 3, DType>(s);
     Tensor<xpu, 3, DType> temp_space = ctx.requested[mboxdet_enum::kTempSpace]
       .get_space_typed<xpu, 3, DType>(out.shape_, s);
     out = -1.f;
     MultiBoxDetectionForward(out, outanchor, cls_prob, loc_pred, anchors, temp_space,
