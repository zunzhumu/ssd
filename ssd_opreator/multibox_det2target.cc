 ...
 * \brief MultiBoxDet2Target op
 * \author Joshua Zhang
*/
#include <algorithm>
#include "./multibox_det2target-inl.h"
...
template<typename DType>
inline void MultiBoxDet2TargetForward(const Tensor<cpu, 2, DType> &loc_target,
  ...
  const int num_anchors = cls_preds.size(2);
  ...
  for (int nbatch = 0; nbatch < num_batches; ++nbatch) {
    ...
    const DType *p_anchor = anchors.dptr_ + nbatch * num_anchors * 4;
    int num_valid_gt = 0;
...
namespace op {
template<>
Operator *CreateOp<cpu>(MultiBoxDet2TargetParam param, int dtype) {
  Operator *op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new MultiBoxDet2TargetOp<cpu, DType>(param);
  });
  return op;
}

Operator* MultiBoxDet2TargetProp::CreateOperatorEx(Context ctx, std::vector<TShape> *in_shape,
...
DMLC_REGISTER_PARAMETER(MultiBoxDet2TargetParam);
MXNET_REGISTER_OP_PROPERTY(_contrib_MultiBoxDet2Target, MultiBoxDet2TargetProp)
...
.add_arguments(MultiBoxDet2TargetParam::__FIELDS__());
}  // namespace op
}  // namespace mxnet
