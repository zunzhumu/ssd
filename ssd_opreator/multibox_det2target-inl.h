 ...
 * \author Joshua Zhang
*/
#ifndef MXNET_OPERATOR_CONTRIB_MULTIBOX_DET2TARGET_INL_H_
#define MXNET_OPERATOR_CONTRIB_MULTIBOX_DET2TARGET_INL_H_
...
}// namespace mshadow-op


namespace mboxtarget_enum {
enum MultiBoxDet2TargetOpInputs {kAnchor, kLabel, kClsPred};
enum MultiBoxDet2TargetOpOutputs {kLoc, kLocMask, kCls};
enum MultiBoxDet2TargetOpResource {kTempSpace};
}  // namespace mboxtarget_enum

struct MultiBoxDet2TargetParam : public dmlc::Parameter<MultiBoxDet2TargetParam> {
  ...
  DMLC_DECLARE_PARAMETER(MultiBoxDet2TargetParam) {
...
};  // struct MultiBoxDet2TargetParam

template<typename xpu, typename DType>
class MultiBoxDet2TargetOp : public Operator {
 public:
  explicit MultiBoxDet2TargetOp(MultiBoxDet2TargetParam param) {
    this->param_ = param;
  }
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    Tensor<xpu, 3, DType> anchors = in_data[mboxtarget_enum::kAnchor]
      .get<xpu, 3, DType>(s);
    ...
    index_t num_anchors = anchors.size(1);
    index_t num_labels = labels.size(1);//58
    // anchor (1, 2268, 4)
    // loc_pred (batch_size, 2268*4)
    // cls_pred(batch_size, 21, 2268)
    ...
    temp_space = -1.0f;
    CHECK_EQ(loc_preds.CheckContiguous(), true);
    ...
    // this anchor is det (batch_size, num_anchor, 6)
    Shape<3> temp_reshape_anchors = Shape3(num_batches, num_anchors, 1);
    temp_space[1] = broadcast_keepdim(reshape(slice<2>(anchors, 2, 3), temp_reshape_anchors), 2, num_labels);
    temp_space[2] = broadcast_keepdim(reshape(slice<2>(anchors, 3, 4), temp_reshape_anchors), 2, num_labels);
    temp_space[3] = broadcast_keepdim(reshape(slice<2>(anchors, 4, 5), temp_reshape_anchors), 2, num_labels);
    temp_space[4] = broadcast_keepdim(reshape(slice<2>(anchors, 5, 6), temp_reshape_anchors), 2, num_labels);
    // label (batch_size, 58, 6)
    ...
    MultiBoxDet2TargetForward(loc_target, loc_mask, cls_target,
 ...
 private:
  MultiBoxDet2TargetParam param_;
};  // class MultiBoxDet2TargetOp

template<typename xpu>
Operator* CreateOp(MultiBoxDet2TargetParam param, int dtype);

#if DMLC_USE_CXX11
class MultiBoxDet2TargetProp : public OperatorProperty {
 public:
  ...
  bool InferShape(std::vector<TShape> *in_shape,
    ...
    CHECK_EQ(ashape[2], 6) << "det dimension should be 4: [id-preds-xmin-ymin-xmax-ymax]";
    ...
    TShape loc_shape = Shape2(lshape[0], ashape[1]*4);  // batch - (num_box * 4)
  ...
  OperatorProperty* Copy() const override {
    MultiBoxDet2TargetProp* MultiBoxDet2Target_sym = new MultiBoxDet2TargetProp();
    MultiBoxDet2Target_sym->param_ = this->param_;
    return MultiBoxDet2Target_sym;
  }

  std::string TypeString() const override {
    return "_contrib_MultiBoxDet2Target";
  }
