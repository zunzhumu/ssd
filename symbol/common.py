    ...
    for k, params in enumerate(zip(from_layers, num_filters, strides, pads)):
        ...
        if from_layer.strip():
            ...
            layer = conv_act_layer(body[-(k+1)], 'multi_feat_%d_conv_3x3' % (k),
                                      num_filter, kernel=(1, 1), pad=(0, 0), stride=(1, 1), act_type='relu')
            layers.append(layer)
        else:
            # attach from last feature layer
            assert len(layers) > 0
            ...
            UpSampling_2 = mx.symbol.UpSampling(conv_1x1, scale=2, sample_type='nearest',
                                       name='multi_feat_%d_UpSampling_2' % (k), workspace=256)

            conv_branch = conv_act_layer(body[-(k+1)], 'multi_feat_%d_conv_branch1' % (k),
                num_1x1, kernel=(3, 3), pad=(0, 0), stride=(2, 2), act_type='relu')

            crop_UpSampling_2 = mx.symbol.Crop(*[UpSampling_2, conv_branch], name='multi_feat_%d_UpSampling_crop' % (k))
            conv = mx.symbol.ElementWiseSum(*[crop_UpSampling_2, conv_branch], name='multi_feat_%d_ElementWiseSum' % (k))
            conv_3x3 = conv_act_layer(conv, 'multi_feat_%d_conv_3x3' % (k),
                                      num_filter // 2, kernel=(3, 3), pad=(1, 1), stride=(1, 1), act_type='relu')
